...
export class Sandbox {
  ...
  exec () {
    if (this.opts.noSandbox) {
      this.process = doSpawn(this.opts, process.execPath, [this.runtimePath, this.opts.runtime, this.scriptPath, '--no-sandbox'])
    } else {
      switch (process.platform) {
        case 'darwin':
          ...
          break
        case 'linux':
          this.process = doSpawn(this.opts, process.execPath, [this.runtimePath, this.opts.runtime, this.scriptPath])
          break
        default:
...
; nodejs needs to read the runtime script as well as the node_modules dir
...
(allow process-exec (literal "${process.execPath}"))`
  ...
  const childProcess = doSpawn(sbx.opts, 
    ...
    [
      '-p', profile,
      process.execPath, sbx.runtimePath, sbx.opts.runtime, sbx.scriptPath
    ]
  )
  return childProcess
}
